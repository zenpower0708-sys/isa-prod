...
const newContent = `function getCertTemplate(certData) {
...
const pattern = /function getCertTemplate\(certData\) \{[\s\S]*?\n\}/;
content = content.replace(pattern, newContent);

fs.writeFileSync(originalFile, content, 'utf8');

let dummyPreview = newContent.split('`')[1];
dummyPreview = dummyPreview.replace('${photoHtml}', '<span class="photo-label">사진 없음</span>');
dummyPreview = dummyPreview.replace('${certData.issueDate}', '2026년 4월 12일');
dummyPreview = dummyPreview.replace('${certData.name}', '홍길동');
dummyPreview = dummyPreview.replace('${levelLabel}', '1급 (Professional)');
dummyPreview = dummyPreview.replace('${certData.discipline}', 'Wave Surfing');
dummyPreview = dummyPreview.replace("${activeTier === 'bronze' ? 'active' : ''}", '');
dummyPreview = dummyPreview.replace("${activeTier === 'silver' ? 'active' : ''}", '');
dummyPreview = dummyPreview.replace("${activeTier === 'gold' ? 'active' : ''}", '');
dummyPreview = dummyPreview.replace("${activeTier === 'diamond' ? 'active' : ''}", 'active');
dummyPreview = dummyPreview.replace('${certData.certNumber}', 'ISA-2026-000001');
dummyPreview = dummyPreview.replace('${qrUrl}', 'https://chart.googleapis.com/chart?chs=180x180&cht=qr&chl=isa-web-portal.vercel.app&choe=UTF-8');
dummyPreview = dummyPreview.replace('\\${x * 6}', '${x * 6}');
dummyPreview = dummyPreview.replace('\\${-y * 4}', '${-y * 4}');

fs.writeFileSync('cert-preview.html', dummyPreview, 'utf8');
